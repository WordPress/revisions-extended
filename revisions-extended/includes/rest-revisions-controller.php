...
class REST_Revisions_Controller extends WP_REST_Revisions_Controller {
	...
	public function get_item( $request ) {
		...
		if ( ! in_array( $revision->post_status, wp_list_pluck( get_revision_statuses(), 'name' ), true ) ) {
			return new WP_Error(
				...
				__( 'This type of revision cannot be retrieved with this endpoint. Use the Core endpoint instead.', 'revisions-extended' ),
				array( 'status' => 403 )
			);
		}
	...
	public function update_item_permissions_check( $request ) {
		...
		if ( ! current_user_can( 'edit_post', $revision->ID ) ) {
			return new WP_Error(
				'rest_cannot_edit',
				__( 'Sorry, you are not allowed to edit revisions for this post.', 'revisions-extended' ),
	...
	public function update_item( $request ) {
		...
		// The id property will cause `parent_controller->prepare_item_for_database` to throw an error because
		// we're editing a revision, which is a different post type than the parent controller expects.
		$request['revision_id'] = $request['id'];
		unset( $request['id'] );

		$revision = $this->get_revision( $request['revision_id'] );
		if ( is_wp_error( $revision ) ) {
			return $revision;
		}
		...
		// Make sure these values are correct after parent controller's `prepare_item_for_database` call.
		$prepared_post->ID        = $revision->ID;
		$prepared_post->post_type = 'revision';
